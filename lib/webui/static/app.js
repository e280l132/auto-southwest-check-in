...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Select-all lives per watch card, so scope it to its own form rather than the whole page
    document.querySelectorAll(".select-all-flights").forEach((toggle) => {
        const form = toggle.closest("form");
        if (!form) return;

        const boxes = () => form.querySelectorAll(".flight-select");
